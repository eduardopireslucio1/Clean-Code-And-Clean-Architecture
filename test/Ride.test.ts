...
test("Deve retornar -2 se a data for inválida", function () {
    const ride = new Ride();
    expect(() => ride.addSegment(10, new Date("typescript"))).toThrow(new Error("Invalid date"))
})
...
test("Deve fazer o cálculo do preço de uma corrida com múltiplos segmentos", function () {
    const ride = new Ride();
    ride.addSegment(10, new Date("2021-03-07T10:00:00"))
    ride.addSegment(10, new Date("2021-03-07T12:00:00"))
    expect(ride.calculate()).toBe(58);
})
